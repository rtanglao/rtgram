...
ARGF.each do |valid_jpeg_filename|
  m1 = /^\d{7}-(\w+)-/.match(valid_jpeg_filename)
  instagram_id =m1[1]
  ...
  top5colours = Miro::DominantColors.new(valid_jpeg_filename.chomp).to_rgb
  pp top5colours
  extraMetadata = { "valid150x150jpg" => true, "id" => instagram_id,
                    "top5colours" => top5colours}
